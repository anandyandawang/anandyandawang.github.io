...
- wires the play court's two sticks, shown on touch devices only: `wireStick` captures the pointer, clamps the offset from the stick's centre to `STICK_RADIUS` (30 px) and writes it to the knob's `--dx` and `--dy`. The move stick walks when `|dx|` passes `STICK_DEADZONE` (12 px) and requests a jump each time `dy` crosses into `dy < -STICK_DEADZONE`; the shot stick loads `STICK_SHOTS[stickDirection(offset)]`, the dominant axis outside the deadzone, and `null` inside it or on release. A stick follows the one pointer it captured: a second finger on the same ring is ignored until that one lifts. It lets go on `pointerup`, `pointercancel` and `lostpointercapture`, and a fresh finger takes the ring whenever the pointer it remembers is gone (the ring no longer holds capture of it, or the same id presses again), so the swallowed touch of the bug report no longer leaves the ring deaf. `wireStick` returns a `release` that recentres the knob and sends the same `null` a lift does; `wirePlayerSticks` calls it on both sticks when the window blurs, when the page is hidden, and whenever a text selection with something in it appears anywhere on the page (`selectionchange`), which is how the page hears that iOS has taken a touch for the selection loupe. The sticks row also swallows `contextmenu`, so a long press on a ring never opens a menu;
...
Each canvas sits inside `div.court-frame`, which carries the border, background and a native CSS `resize: both` grip. A `ResizeObserver` in the engine re-measures the canvas, recomputes ranges and homes, and keeps play going through the resize. The wide frame lives in a full-bleed wrapper so it can grow past the text column. The play canvas's own focus outline is off and `.court-frame:focus-within` turns the frame border `--sage` instead, so the frame is the focus cue. The play frame also drops the crosshair for the default cursor, because its pointer is off and there is nothing to aim at. On touch devices the whole play section (`.play-wrap`) is `user-select: none` with the touch callout off, so a thumb resting on a ring never starts a text selection; the reduced-motion block sets both back to their defaults, since nothing is held there.
