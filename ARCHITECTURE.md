...
A placement may name a `holder`: the cat the served bird waits at. `serve` stores it on `bird.holder`, and every sub-step a held bird has a holder it is pinned to `racketPoint(holder)` with no velocity, so the bird walks with the cat. `racketPoint` is the plain ground geometry, so a held bird does not rise with a jumping holder. A bird held by the player stays held until the player loads a shot (`holder.input.shot !== null`), on top of the usual `hold` and gate rules; a bird held by an AI cat is held only by those. The first sub-step a bird is not held, `bird.holder` is cleared and the bird falls from the racket as any served bird does.
...
- `state.birds`: `{ x, y, vx, vy, inPlay, holdUntil, respawnAt, strikes, shot, crossing, receiver, striker, holder }` plus effect timers. `strikes` counts hits since the last serve, reset in `serve`, incremented in `strike` after the shot strategy has run. `shot` is the `kind` string of the last strike, or `null`. `striker` is the cat that last hit this bird, cleared once the bird leaves that cat's reach (so a cat cannot re-hit its own falling smash) and cleared in `serve` and when the cat is removed. `holder` is the cat a served bird waits at, taken from the placement in `serve`, cleared the first sub-step the bird is no longer held, in `strike`, and when the holder is removed. `state.bird` is a getter for the first bird in play, kept for tests.
...
The serve waits. `matchServe` names the server as the bird's `holder`, so when the player is the server the bird sits at its racket and walks with it until a shot is loaded; then it drops and is struck with that shot.

The racket says what is loaded. `restRacketAngle` gives the player `IDLE_RACKET_ANGLE` (`0.2`, the racket hanging) with nothing loaded, and otherwise `READY_ANGLES[input.shot]`: `-0.2` for a drop, `RACKET_REST_ANGLE` for a drive, `-0.8` for a clear, `OVERHEAD_ANGLE` for a smash. AI cats keep the old overhead-or-rest angle, and the swing interpolation and the cheer pump are unchanged.
